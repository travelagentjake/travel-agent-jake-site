    ...
    <div class="dd-calculator" style="margin-top:24px;">
      ...
      <div class="dd-field-grid" style="margin-top:18px;">
        <div class="dd-field" id="ddFirstPaymentWrap">
          <label for="ddFirstPayment">Your preferred first payment date</label>
          <input type="date" id="ddFirstPayment">
          <div class="dd-field-hint">Must be at least 4 weeks from today.</div>
        </div>
        <div class="dd-field" id="ddFullPaymentWrap" style="display:none;">
          <label for="ddFullPaymentDate">Your preferred payment date</label>
          <input type="date" id="ddFullPaymentDate">
          <div class="dd-field-hint dd-field-hint-strong" id="ddFullPaymentHint">Must be at least 4 weeks from today, and on or before your balance due date.</div>
        </div>
      </div>
...
(function(){
  ...
  var firstPaymentEl = document.getElementById('ddFirstPayment');
  var fullPaymentEl = document.getElementById('ddFullPaymentDate');
  ...
  function recalc(){
    ...
    if (!travelDate || !balance || balance <= 0){
      ...
      fullPaymentHint.textContent = 'Must be at least 4 weeks from today, and on or before your balance due date.';
      fullPaymentEl.removeAttribute('min');
      fullPaymentEl.removeAttribute('max');
      return;
    }
    ...
    var minFirstPayment = addDays(todayMidnight, 28);

    fullPaymentHint.innerHTML = 'Must be on or before your balance due date of <b>' + fmtDate(finalDueDate) + '</b>, and at least 4 weeks from today.';
    fullPaymentEl.min = minFirstPayment.toISOString().slice(0, 10);
    fullPaymentEl.max = finalDueDate.toISOString().slice(0, 10);

    if (mode === 'monthly'){
      var firstPayment = parseDate(firstPaymentEl);
      if (!firstPayment){
        setStatus('dd-status-wait', 'Choose your preferred first payment date to see your plan.');
        return;
      }
      if (firstPayment < minFirstPayment){
        setStatus('dd-status-bad', 'Your first payment needs to be at least 4 weeks from today, the earliest date you can choose is ' + fmtDate(minFirstPayment) + '.');
        return;
      }
      if (firstPayment >= finalDueDate){
        setStatus('dd-status-bad', 'Your first payment date is too close to your balance due date (' + fmtDate(finalDueDate) + '). Try an earlier first payment date, or pay in full on one date instead.');
        return;
      }

      var totalDays = daysBetween(firstPayment, finalDueDate);
      var instalments = Math.max(1, Math.ceil(totalDays / 28));
      var baseAmount = Math.floor((balance / instalments) * 100) / 100;
      var rows = [];
      var runningTotal = 0;
      for (var i = 0; i < instalments; i++){
        var payDate = addDays(firstPayment, i * 28);
      ...
      summaryEl.innerHTML =
        ...
        '<div class="dd-result-tile"><b>' + fmtDate(finalDueDate) + '</b><span>Final payment date</span></div>';
      ...
      var fullDate = parseDate(fullPaymentEl);
      if (!fullDate){
        setStatus('dd-status-wait', 'Choose your preferred payment date to see your plan.');
        return;
      }
      if (fullDate < minFirstPayment){
        setStatus('dd-status-bad', 'Your payment date needs to be at least 4 weeks from today, the earliest date you can choose is ' + fmtDate(minFirstPayment) + '.');
        return;
      }
      if (fullDate > finalDueDate){
        setStatus('dd-status-bad', 'Your payment date needs to be on or before your balance due date (' + fmtDate(finalDueDate) + ').');
        return;
      }
  ...
  [travelDateEl, balanceEl, firstPaymentEl, fullPaymentEl].forEach(function(el){
...
<section class="theme-light" style="padding-top:0;">
  <div class="wrap">
    ...
    <p style="margin-top:14px;">Alongside the biometric registration, a border officer may still ask the same questions they always have: your purpose of visit, how long you're staying, where you're staying, proof of funds for your trip, and your return ticket. This isn't new to EES, it's standard Schengen entry practice that now happens alongside the biometric step rather than instead of it.</p>
...
<section class="theme-light" style="padding-top:0;">
  <div class="wrap">
    ...
    <p style="margin-top:14px;">Officially, you can decline to give your biometric data, but the EU is explicit that refusing it means you'll be denied entry. It isn't really an option if you actually want to get in, biometric registration is a mandatory condition of entry under EES for anyone it applies to.</p>
    <p style="margin-top:14px;">The standard border questions work differently. There's no fixed penalty for any single answer, but an officer who isn't satisfied you meet the conditions to enter (enough money for your trip, a genuine return date, evidence you're not intending to work illegally, and so on) can pull you aside for further questioning, ask for more documentation on the spot, or ultimately refuse entry and send you back on the next available transport. A Schengen-eligible passport or a valid ETIAS, once it exists, gets you to the border, it doesn't guarantee an officer lets you through if something doesn't add up.</p>
    {jake_tip("Keep it simple and have the basics ready: your accommodation address, a rough idea of your budget for the trip, and your return flight details. Vague or inconsistent answers are what tend to trigger a longer conversation, not the questions themselves.")}
  </div>
</section>
